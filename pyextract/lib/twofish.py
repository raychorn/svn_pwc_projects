...
from misc import __int__
from misc import xrange

from misc import str_to_bytes
from misc import pack_bytes_into
...
class Twofish:
    ...
    def set_key(self, key):
        ...
        i = 0
        while key:
            key_word32[i] = struct.unpack("<L", str_to_bytes(key)[0:4])[0]
            key = key[4:]
            i += 1
    ...
    def decrypt(self, block):
        ...
        while block:
            a, b, c, d = struct.unpack("<4L", str_to_bytes(block[:16]))
            temp = [a, b, c, d]
            decrypt(self.context, temp)
            plaintext = pack_bytes_into(plaintext, temp)
            block = block[16:]
    ...
    def encrypt(self, block):
        ...
        while block:
            a, b, c, d = struct.unpack("<4L", str_to_bytes(block[0:16]))
            temp = [a, b, c, d]
            encrypt(self.context, temp)
            ciphertext = pack_bytes_into(ciphertext, temp)
            block = block[16:]
...
def set_key(pkey, in_key, key_len):
    ...
    for i in xrange(pkey.k_len):
        if WORD_BIGENDIAN:
            ...
            b = in_key[i + i + 1]
        mo_key[i] = b
        pkey.s_key[__int__(pkey.k_len) - i - 1] = mds_rem(a, b);
