...
from ..connect.oracle import OracleMessenger

from pyextract import config

LOGGER = multiprocessing.get_logger()

class ODBCStream(object):
    ...
    def get_dot_parameters_from(self, parameters):
        from lib.parameters import ParametersProcessor
        p = ParametersProcessor()
        p.on(parameters=parameters)
        return p.new_parameters
    
    def generate_sql_statements(self):
        for item in self.__dot__:
            if (len(item) == 0):
                continue
            
            yield ''.join([self.base_query, ' WHERE %s' % (' AND '.join(item))])
    ...
    def start(self, queue: multiprocessing.Queue, metadata=None):
        ...
        self.__dot__ = self.get_dot_parameters_from(metadata.parameters.get('Parameters', []))

        from lib.sqlutils import SQLCoder
        sql = SQLCoder(metadata.parameters, ecf_data=metadata.ecf_data)
        self.base_query = sql.make_sql_statement()
        
        for statement in self.generate_sql_statements():

            def read_data_using(self, sql_statement):
                self.messenger.begin_extraction(sql_statement, self.chunk_size)
                ...
                while (1):
                    if self.is_stopped():
                        ...
                        if (data):
                            ...
                            item = (True, data, metadata, sql_statement,)
                            self.queue.put(item)
                            ...
                            self.logger.info(
                                "0 new records read from source database ({:,} total)."
                                    .format(self.rows_read)
                            )
                            break
            ...
            if self.is_stopped():
                ...
                read_data_using(self, statement)
            
        ## Create Thread-safe generator that fans out queries

        ## Create Threads to...
        ##### Fetch next query (Skipping previously extracted)
        ##### Execute query
        ##### Put Data on tuple on Write Queue as follows: (data: list, metadata: utils.DataDefinition, query_text: str = None)
        ##### Add query to sub-query status tracker
